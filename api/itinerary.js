...
const DEFAULT_ITINERARY = require('./itinerary-seed.json');
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    try {
      const { blobs } = await list({ prefix: 'itinerary/', storeId });
      if (!blobs.length) {
        res.status(200).json({ data: DEFAULT_ITINERARY, updatedAt: null });
        return;
      }
      ...
      res.status(200).json({ data: DEFAULT_ITINERARY, updatedAt: null, warning: err.message });
    }
    return;
  }
  ...
  if (req.method === 'POST') {
    try {
      ...
      const filename = `itinerary/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.json`;
